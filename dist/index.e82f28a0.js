...
const draw = (0, _svgJs.SVG)().addTo(".container").size("100%", "100%");
...
rectbtn.addEventListener("click", function(e) {
    drawing = !drawing;
    if (drawing) draw.on("click", startDraw);
    else draw.off("click");
});
...
const drawer = function() {
    ...
    let box;
    rect = !rect;
    if (rect) draw.on("mousemove", function(e) {
        x = left;
        y = top;
        if (left > e.offsetX) {
            x = e.offsetX;
...
},{"@svgdotjs/svg.js":"9S56O","@svgdotjs/svg.draggable.js":"af8Vx"}],"9S56O":[function(require,module,exports) {
...
function registerMethods(name, m) {
    ...
    if (typeof name === "object") {
        for(const _name1 in name)registerMethods(_name1, name[_name1]);
        return;
    }
...
function css(style, val) {
    ...
    if (arguments.length < 2) {
        ...
        if (typeof style === "object") for(const name1 in style)// set empty string if null/undefined/'' was given
        this.node.style[camelCase(name1)] = style[name1] == null || isBlank.test(style[name1]) ? "" : style[name1];
...
class Color {
  ...
  */ static random(mode = "vibrant", t, u) {
        ...
        if (mode === "vibrant") {
            ...
            const color1 = new Color(r, g, b);
            return color1;
        } else if (mode === "pastel") {
            const l1 = 8 * random() + 86;
            const c1 = 17 * random() + 9;
            const h1 = 360 * random();
            const color2 = new Color(l1, c1, h1, "lch");
            return color2;
        } else if (mode === "dark") {
            const l2 = 10 + 10 * random();
            const c2 = 50 * random() + 86;
            const h2 = 360 * random();
            const color3 = new Color(l2, c2, h2, "lch");
            return color3;
        } else if (mode === "rgb") {
            const r1 = 255 * random();
            const g1 = 255 * random();
            const b1 = 255 * random();
            const color4 = new Color(r1, g1, b1);
            return color4;
        } else if (mode === "lab") {
            const l3 = 100 * random();
            const a = 256 * random() - 128;
            const b2 = 256 * random() - 128;
            const color5 = new Color(l3, a, b2, "lab");
            return color5;
        } else if (mode === "grey") {
            const grey = 255 * random();
            const color6 = new Color(grey, grey, grey);
            return color6;
    ...
    init(a = 0, b = 0, c = 0, d = 0, space = "rgb") {
        ...
        if (typeof a === "number") {
            ...
            if (isRgb.test(a)) {
                ...
                const [, _a1, _b1, _c1] = hex.exec(sixDigitHex(a)).map(hexParse);
                Object.assign(this, {
                    _a: _a1,
                    _b: _b1,
                    _c: _c1,
        ...
        const { _a: _a2 , _b: _b2 , _c: _c2 , _d  } = this;
  ...
  */ rgb() {
        if (this.space === "rgb") return this;
            ...
            const bU = x * 0.0557 + y * -0.204 + z * 1.0570; // Convert the values to true rgb values
            const pow = Math.pow;
            const bd = 0.0031308;
            const r = rU > bd ? 1.055 * pow(rU, 1 / 2.4) - 0.055 : 12.92 * rU;
            const g = gU > bd ? 1.055 * pow(gU, 1 / 2.4) - 0.055 : 12.92 * gU;
            const b1 = bU > bd ? 1.055 * pow(bU, 1 / 2.4) - 0.055 : 12.92 * bU; // Make and return the color
            const color = new Color(255 * r, 255 * g, 255 * b1);
            return color;
        } else if (this.space === "hsl") {
            // https://bgrins.github.io/TinyColor/docs/tinycolor.html
            // Get the current hsl values
            let { h: h1 , s , l: l1  } = this;
            h1 /= 360;
            s /= 100;
            l1 /= 100; // If we are grey, then just make the color directly
            if (s === 0) {
                l1 *= 255;
                const color1 = new Color(l1, l1, l1);
                return color1;
            } // TODO I have no idea what this does :D If you figure it out, tell me!
            const q = l1 < 0.5 ? l1 * (1 + s) : l1 + s - l1 * s;
            const p = 2 * l1 - q; // Get the rgb values
            const r1 = 255 * hueToRgb(p, q, h1 + 1 / 3);
            const g1 = 255 * hueToRgb(p, q, h1);
            const b2 = 255 * hueToRgb(p, q, h1 - 1 / 3); // Make a new color
            const color2 = new Color(r1, g1, b2);
            return color2;
        } else if (this.space === "cmyk") {
            // https://gist.github.com/felipesabino/5066336
            // Get the normalised cmyk values
            const { c: c1 , m , y: y1 , k  } = this; // Get the rgb values
            const r2 = 255 * (1 - Math.min(1, c1 * (1 - k) + k));
            const g2 = 255 * (1 - Math.min(1, m * (1 - k) + k));
            const b3 = 255 * (1 - Math.min(1, y1 * (1 - k) + k)); // Form the color and return it
            const color3 = new Color(r2, g2, b3);
            return color3;
        } else return this;
    }
    toArray() {
...
class Timeline extends EventTarget {
    ...
    schedule(runner, delay, when) {
        ...
        this._runners.push(runnerInfo1);
    ...
    _stepFn(immediateStep = false) {
        ...
        for(let i = 0, len = this._runners.length; i < len; i++){
            // Get and run the current runner and ignore it if its inactive
            const runnerInfo1 = this._runners[i];
            const runner1 = runnerInfo1.runner;
            let dt = dtTime; // Make sure that we give the actual difference
            // between runner start time and now
            const dtToStart1 = this._time - runnerInfo1.start; // Dont run runner if not started yet
            if (dtToStart1 <= 0) {
                runnersLeft = true;
                continue;
            } else if (dtToStart1 < dt) // Adjust dt to make sure that animation is on point
            dt = dtToStart1;
            if (!runner1.active()) continue; // If this runner is still going, signal that we need another animation
            // frame, otherwise, remove the completed runner
            const finished = runner1.step(dt).done;
            if (!finished) runnersLeft = true; // continue
            else if (runnerInfo1.persist !== true) {
                // runner is finished. And runner might get removed
                const endTime = runner1.duration() - runner1.time() + this._time;
                if (endTime + runnerInfo1.persist < this._time) {
...
},{}],"af8Vx":[function(require,module,exports) {
var _svgJs = require("@svgdotjs/svg.js");
...
class DragHandler {
    ...
    startDrag(ev) {
        ...
        if (isMouse && (ev.which || ev.buttons) !== 1) return;
        ...
        (0, _svgJs.on)(window, eventMove, this.drag);
        (0, _svgJs.on)(window, eventEnd, this.endDrag);
    ...
    drag(ev) {
        ...
        const newBox = new (0, _svgJs.Box)(x, y, box.w, box.h);
...
},{"@svgdotjs/svg.js":"9S56O"}]},["eZyLq","dV6cC"], "dV6cC", "parcelRequire9744")
